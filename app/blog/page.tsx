    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
       ...
       <div className="max-w-7xl mx-auto px-6 py-20">
          ...
          <div className="flex justify-center gap-4">
             {page > 1 && (
               <Link href={`/blog?page=${page - 1}`} className="flex items-center gap-2 px-6 py-3 bg-white border rounded-lg hover:bg-slate-50"><ChevronLeft size={16}/> Previous</Link>
             )}
             {page < totalPages && (
               <Link href={`/blog?page=${page + 1}`} className="flex items-center gap-2 px-6 py-3 bg-white border rounded-lg hover:bg-slate-50">Next <ChevronRight size={16}/></Link>
             )}
          </div>
       </div>
    </div>
